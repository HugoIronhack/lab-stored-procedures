...
CREATE PROCEDURE CustomersByAction()
BEGIN
...
CREATE PROCEDURE CustomersByCategory(IN categoryName VARCHAR(255))
BEGIN
    SELECT first_name, last_name, email
    FROM customer
    JOIN rental ON customer.customer_id = rental.customer_id
...
CREATE PROCEDURE CountingFilms(IN minFilmCount INT)
BEGIN
    SELECT category.name, COUNT(*) AS film_count
    FROM category
    JOIN film_category ON category.category_id = film_category.category_id
    GROUP BY category.name
    HAVING COUNT(*) > minFilmCount;
END //

DELIMITER ;


CALL CountingFilms(5);
